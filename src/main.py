...
def plot_kdistance_curve(X: np.ndarray, k: int = 5, n_max: int = 10_000) -> None:
    ...
    plt.axhline(cutoff, ls="--", color="red",
                label=f"mediana + σ = {cutoff:.4f}")
    plt.xlabel("Pontos ordenados"); plt.ylabel(f"{k}ª distância")
...
def avaliar_dbscan(X, y_true, eps, min_samples):
    ...
    return {"nome": f"DBSCAN (eps={eps:.3f})", "cm": cm, "prec": p,
            "rec": r, "f1": f1, "mcc": mcc, "auprc": ap, "scores": scores}


def _scores_hdb(modelo: HDBSCAN) -> np.ndarray:
    return (1.0 - modelo.probabilities_
...
def avaliar_hdbscan(X, y_true, min_cluster_size, q_out):
    ...
    return {"nome": f"HDBSCAN (min_sz={min_cluster_size})", "cm": cm, "prec": p,
            "rec": r, "f1": f1, "mcc": mcc, "auprc": ap, "scores": scores}
...
def plot_matriz(cm, titulo, ax=None):
    ...
    ax.set_title(titulo)
    ax.set_xticks([0, 1]); ax.set_xticklabels(["Legítimo", "Fraude"])
    ax.set_yticks([0, 1]); ax.set_yticklabels(["Legítimo", "Fraude"])
    for i in range(2):
        for j in range(2):
...
def main(argv: List[str] | None = None) -> None:
    ...
    parser.add_argument("--arquivo", type=Path, default="src\datasets\creditcardfraud_modified.csv")
    ...
    for r in resultados:
        print(f"\n===> {r['nome']}")
        print(pd.DataFrame(r["cm"], index=["Real 0", "Real 1"],
